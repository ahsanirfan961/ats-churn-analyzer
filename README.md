...
The frontend surfaces the flagged claims and the rewrite — the self-check is a feature to show,
...
- The agent is told to refuse questions about fields the dataset doesn't have (region, revenue
  trend, product category) rather than answering against the nearest similar column. The eval set
  has a trap question for exactly this.
- `run_pandas` validates against an AST **whitelist** before executing, not a regex blocklist,
  which is easy to route around. Import statements, dunder attribute access, and
  `eval`/`exec`/`open`/`__import__` are all rejected pre-execution, and the sandbox gets a copy of
  the frame so it cannot mutate shared state.
...
```
notebooks/            Stage 1 notebook
backend/app/
  artifacts/          churn_model.joblib + customer_churn_clean.csv
  model/              loader, predict (risk / hypothetical / scenarios)
  data/               store (precomputed decile + segment tables), describe,
                      segment, rank, correlate, enrich
  sandbox/            AST-validated run_pandas
  tools/              pydantic schemas + LangChain registry (the 8 tools)
  verification/       faithfulness judge + verdict models
  agent/              prompts, turn-window summarization, graph, verified turn
  db/                 sqlite checkpointer + thread metadata
  routes/             chat (SSE) + threads
backend/tests/        unit tests + eval_set.py
frontend/src/         chat client, streaming hook, 5 components
docs/eval_report.md   generated by tests/eval_set.py
```
...
Claude (Claude Code) was used throughout: to draft the notebook's narrative sections, to write
most of the application code from a phase-by-phase plan I wrote first, and to draft this README.
The design decisions — tool granularity, the verification approach and the two rejected
